...
13. tried_to_apply — FAILED apply attempts only. Yes ONLY when BOTH are true: (i) the USER EXPLICITLY said to apply / gave clear affirmative consent to apply for a specific job ("haan apply karo", "yes apply kar do", "ಅಪ್ಲೈ ಮಾಡಿ", etc.) — the bot invoking the apply_jobs tool on its own is NOT sufficient; the user must have explicitly asked. AND (ii) the application did NOT succeed — either the apply_jobs API was never actually called, OR it was called and explicitly failed (there is no "अप्लाई हो गया है" / "apply ho gaya" / "application submitted" confirmation, or the bot acknowledged an error). If the application succeeded (applied_to_job=Yes), this MUST be No. If the user never explicitly said to apply, this is No.
